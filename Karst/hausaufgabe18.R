# Hausaufgabe 18
# Lisa Karst <karst@students.uni-marburg.de>
# 2014-06-18
# Dieses Werk ist lizenziert und darf nur zu Prüfungszwecken dienen.
...
lm_x1 <- lm(y ~ x1, data=pyreg)
summary(lm_x1)

lm_x2 <- lm(y ~ x2, data=pyreg)
summary(lm_x2)
...
lm_x1x2 <- lm(y ~ x1 + x2, data=pyreg)
summary(lm_x1x2)

# Wie gut passt das lineare Modell zu den Daten? Schauen Sie sich die R^2 (0.9582= sehr hoch) und 
# F-Werte (p-value: 7.356e-13) an sowie auch die t-Werte für die einzelnen Prediktoren. Glauben Sie, 
# dass y im linearen Verhältnis zu x1 und x2 steht? (Könnte durchaus sein, ich bin mir aber nicht sicher- das ist eher ein Gefühl, als Wissen) Machen Sie eine Grafik wie
...
Ich denke, dass y im linearen Verhältnis zu x2 steht. y wächst mit x2.
